...
int ngli_buffer_vk_init(struct buffer *s, int size, int usage)
{
    ...
    VkResult vkret = vkCreateBuffer(vk->device, &buffer_create_info, NULL, &s_priv->vkbuf);
    if (vkret != VK_SUCCESS)
        return -1;

    VkMemoryRequirements mem_req;
    vkGetBufferMemoryRequirements(vk->device, s_priv->vkbuf, &mem_req);
    ...
    vkret = vkAllocateMemory(vk->device, &alloc_info, NULL, &s_priv->vkmem);
    if (vkret != VK_SUCCESS) {
        LOG(ERROR, "can not allocate memory");
        return NGL_ERROR_MEMORY;
    }

    vkret = vkBindBufferMemory(vk->device, s_priv->vkbuf, s_priv->vkmem, 0);
    if (vkret != VK_SUCCESS) {
...
int ngli_buffer_vk_upload(struct buffer *s, const void *data, int size)
{
    ...
    struct buffer_vk *s_priv = (struct buffer_vk *)s;
    void *mapped_mem;
    VkResult ret = vkMapMemory(vk->device, s_priv->vkmem, 0, size, 0, &mapped_mem);
    if (ret != VK_SUCCESS)
        return -1;
    memcpy(mapped_mem, data, size);
    vkUnmapMemory(vk->device, s_priv->vkmem);
    return 0;
}
...
void ngli_buffer_vk_freep(struct buffer **sp)
{
    ...
    struct vkcontext *vk = gctx_vk->vkcontext;
    struct buffer_vk *s_priv = (struct buffer_vk *)s;
    vkDestroyBuffer(vk->device, s_priv->vkbuf, NULL);
    vkFreeMemory(vk->device, s_priv->vkmem, NULL);
    ngli_freep(sp);
}
