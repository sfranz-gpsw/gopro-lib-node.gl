...
static int gl_init(struct gctx *s)
{
    ...
    struct gctx_gl *s_priv = (struct gctx_gl *)s;
#ifdef ENABLE_CAPTURE
    s_priv->debug_capture = (getenv("DEBUG_CAPTURE") != NULL);
    if (s_priv->debug_capture) init_capture();
#endif
...
#ifdef ENABLE_CAPTURE
    if (s_priv->debug_capture) begin_capture();
#endif
...
#ifdef ENABLE_CAPTURE
    if (s_priv->debug_capture) end_capture();
#endif
