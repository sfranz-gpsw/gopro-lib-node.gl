...
static int rtt_prefetch(struct ngl_node *node)
{
    ...
    for (int i = 0; i < s->nb_color_textures; i++) {
        ...
        for (int j = 0; j < n; j++) {
            if (s->samples) {
                ...
                float *load_value = rt_params.colors[rt_params.nb_colors].load_value;
                memcpy(load_value, s->clear_color, sizeof(s->clear_color));
    ...
    if (s->depth_texture) {
        ...
        if (s->samples) {
            ...
            rt_params.depth_stencil.store_op = NGLI_STORE_OP_DONTCARE;
        } else {
        ...
        if (depth_format != NGLI_FORMAT_UNDEFINED) {
            ...
            rt_params.depth_stencil.load_op = NGLI_LOAD_OP_CLEAR;
            rt_params.depth_stencil.store_op = s->use_rt_resume ? NGLI_STORE_OP_STORE : NGLI_LOAD_OP_DONTCARE;
        }
    }
    ...
    if (s->use_rt_resume) {
        ...
        rt_params.depth_stencil.store_op = s->depth_texture ? NGLI_STORE_OP_STORE : NGLI_LOAD_OP_DONTCARE;
