...
static int build_attribute_descs(struct pipeline *s, const struct pipeline_params *params)
{
    ...
    for (int i = 0; i < params->nb_attributes; i++) {
        ...
        if (!ngli_darray_push(&s_priv->vertex_buffers, &buffer_vk->vkbuf))
            return NGL_ERROR_MEMORY;
...
static int pipeline_graphics_init(struct pipeline *s, const struct pipeline_params *params)
{
    ...
    ret = ngli_vk_create_renderpass_info(s->gctx, &graphics->rt_desc, &render_pass, 0);
    if (ret < 0)
        return ret;
    ...
    VkResult vkret = vkCreateGraphicsPipelines(vk->device, NULL, 1, &graphics_pipeline_create_info, NULL, &s_priv->pipeline);
    if (vkret != VK_SUCCESS)
...
void ngli_pipeline_vk_exec(struct pipeline *s)
{
    ...
    if (s->type == NGLI_PIPELINE_TYPE_GRAPHICS) {
        ...
        if (indices) {
            ...
            vkCmdBindIndexBuffer(cmd_buf, indices_vk->vkbuf, 0, indices_type);
