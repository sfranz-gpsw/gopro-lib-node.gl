        ...
        switch (p->type) {
            ...
            case PARAM_TYPE_STR: {
                ...
                if (!strcmp(p->key, "label") &&
                    ngli_is_default_label(node->clazz->name, s))
                    break;
                ngli_bstr_printf(b, " %s:", p->key);
                for (int i = 0; s[i]; i++)
                    if (s[i] >= '!' && s[i] <= '~' && s[i] != '%')
                     ...
                     const struct ngl_node *node)
{
    ...
    if ((ret = serialize_children(nlist, b, node, (uint8_t *)node, ngli_base_node_params)) < 0 ||
        (ret = serialize_children(nlist, b, node, node->priv_data, node->clazz->params)) < 0)
        return ret;

    const uint32_t tag = node->clazz->id;
    ngli_bstr_printf(b, "%c%c%c%c",
                    tag >> 24 & 0xff,
                    tag >> 16 & 0xff,
                    tag >>  8 & 0xff,
                    tag       & 0xff);
    if ((ret = serialize_options(nlist, b, node, node->priv_data, node->clazz->params)) < 0 ||
