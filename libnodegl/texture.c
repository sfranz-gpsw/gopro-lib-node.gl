...
static void texture2d_set_sub_image(struct texture *s, const uint8_t *data, int stride, int row_upload)
{
    struct glcontext *gl = s->gl;
    const struct texture_params *params = &s->params;
...
static void texture3d_set_sub_image(struct texture *s, const uint8_t *data, int stride, int row_upload)
{
    struct glcontext *gl = s->gl;
    const struct texture_params *params = &s->params;
...
static void texturecube_set_sub_image(struct texture *s, const uint8_t *data, int stride, int row_upload)
{
    struct glcontext *gl = s->gl;
    const struct texture_params *params = &s->params;
    ...
    const int face_size = data ? s->bytes_per_pixel * stride * params->height : 0;
    for (int face = 0; face < 6; face++) {
...
static void texture_set_sub_image(struct texture *s, const uint8_t *data, int stride)
{
    struct glcontext *gl = s->gl;
    const struct texture_params *params = &s->params;
    ...
    if (gl->features & NGLI_FEATURE_ROW_LENGTH)
        ngli_glPixelStorei(gl, GL_UNPACK_ROW_LENGTH, stride);
    else if (params->width != stride)
        row_upload = 1;

    switch (s->target) {
    case GL_TEXTURE_2D:
        texture2d_set_sub_image(s, data, stride, row_upload);
        break;
    case GL_TEXTURE_3D:
        texture3d_set_sub_image(s, data, stride, row_upload);
        break;
    case GL_TEXTURE_CUBE_MAP:
        texturecube_set_sub_image(s, data, stride, row_upload);
        break;
    }
...
int ngli_texture_upload(struct texture *s, const uint8_t *data, int stride)
{
    struct glcontext *gl = s->gl;
    const struct texture_params *params = &s->params;
    ...
    ngli_glBindTexture(gl, s->target, s->id);
    if (data) {
        texture_set_sub_image(s, data, stride);
        if (ngli_texture_has_mipmap(s))
